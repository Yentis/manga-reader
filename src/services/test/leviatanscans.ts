...
async function readUrlWrongSeasonOrder (): Promise<void> {
  const url = 'https://leviatanscans.com/eng/manga/survivor-story-of-a-sword-king-in-a-fantasy-world/'
  const manga = await getMangaInfo(url, SITE_TYPE)
  const desired = new Manga(url, SITE_TYPE)
  desired.chapter = 'Season 2 | 100'
  ...
  desired.chapterUrl = 'https://leviatanscans.com/eng/manga/survivor-story-of-a-sword-king-in-a-fantasy-world/season-2/100/'
  desired.chapterNum = 100
...
async function search (): Promise<void> {
  ...
  desired.chapter = '188'
  desired.url = 'https://leviatanscans.com/eng/manga/burning-effect/'
